...
- Use telegram_send when you want to message the user directly on Telegram without them messaging you first.
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params) {
			try {
				const result = await sendProactiveTelegramMessage(config.allowedUserId, params.message);
				return {
	...
	// Manual command for sending proactive messages
	pi.registerCommand("telegram-send", {
		description: "Send a proactive message to the Telegram user",
		handler: async (args: string, ctx) => {
			if (!config.botToken || !config.allowedUserId) {
				ctx.ui.notify("Telegram bot is not configured or user not paired. Run /telegram-setup first.", "error");
				return;
			}
			...
			try {
				const result = await sendProactiveTelegramMessage(config.allowedUserId, args);
